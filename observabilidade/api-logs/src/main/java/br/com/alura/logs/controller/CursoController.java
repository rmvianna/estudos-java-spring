...
public class CursoController {
	...
	@PostMapping
	 public ResponseEntity<Object> saveCurso(@RequestBody @Valid CursoDto cursoDto){
        try {
            logger.debug("Iniciando processo de insercao de registro de novo curso");
            logger.debug("Chamando o cursoService para validar se numero de matricula ja existe");
            ...
            if(cursoService.existsByNumeroMatricula(cursoDto.getNumeroMatricula())) {
                ...
                return ResponseEntity.status(HttpStatus.CONFLICT).body("O número de matrícula do curso já esta em uso!");
            }

            logger.debug("Validacao de numero de matricula concluida");
            logger.debug("Chamando o cursoService para validar se numero de curso ja existe");
            ...
            if(cursoService.existsByNumeroCurso(cursoDto.getNumeroCurso())) {
                ...
                return ResponseEntity.status(HttpStatus.CONFLICT).body("O número do curso já está em uso!");
            }

            logger.debug("Validacao de numero de curso concluida");
            logger.debug("Validacoes de cursoService sobre cursoDTO executadas com sucesso");
            logger.debug("Chamando cursoService.save para armazenar o novo registro");
            ...
            throw new InternalErrorException("Erro momentâneo. Por favor, tente novamente mais tarde");
        }
	}
	...
	public ResponseEntity<Object> getOneCursos(@PathVariable(value="id") UUID id) {
        try {
            logger.debug("Chamando cursoService.findById para buscar um registro por UUID");
            Optional<CursoModel> cursoModelOptional = cursoService.findById(id);
            logger.debug("Validando por cursoModelOptional se o UUID existe");
            ...
            logger.debug("O registro foi encontrado por cursoService no banco de dados!");
